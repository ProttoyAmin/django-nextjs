...
function ShowFollowButtons({
  ...
  variant,
  size,
}: ShowFollowButtonsProps) {
  const dispatch = useAppDispatch();
  const currentUserId = useAppSelector((state) => state.user.currentUser?.id);
  const relationship = useAppSelector(
    selectFollowRelationshipByUserId(Number(targetId)),
  );
  const isLoading = useAppSelector(
    selectFollowLoadingByUserId(Number(targetId)),
  );
  ...
  const handleFollowToggle = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        dispatch(
          setFollowLoading({ userId: Number(targetId), isLoading: false }),
        );
      } else {
        if (previousState) {
          dispatch(
