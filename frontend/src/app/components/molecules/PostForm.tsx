...
import { CreatePostRequest, PostFormType, PostType } from "@/src/types/post";
...
import {
  createPost,
  createImagePostWithFile,
  createVideoPostWithFile,
  createTextPost,
...
function CreatePostForm({ onClose, onSubmit }: CreatePostFormProps) {
  ...
  const handleFormSubmit = useCallback(async (data: PostFormType) => {
    ...
    try {
      ...
      if (finalData.media && finalData.media.length > 0) {
        for (const mediaFile of finalData.media) {
          if (mediaFile.type.startsWith('image/')) {
            const response = await createImagePostWithFile(
        ...
        console.log('calling createtextpost')
  ...
  return (
    <>
      ...
      <div className={`overflow-hidden max-h-[calc(90vh-100px)] relative ${isMounted 
          ? 'bounce-up-active scale-100 opacity-100 translate-y-0' 
          : 'bounce-up scale-95 opacity-0 translate-y-8'
        }`}>
        <div
